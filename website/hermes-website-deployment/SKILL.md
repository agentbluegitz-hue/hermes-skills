...
### 3. Deploy to DreamHost\nDeploy the built site to DreamHost using rsync over SSH.\n\n#### Verify SSH connectivity\n```bash\nssh -i ~/.ssh/id_ed25519 -o BatchMode=yes -o PreferredAuthentications=publickey agent_blue@iad1-shared-b8-40.dreamhost.com true\n```\nIf this fails, diagnose based on the error message:\n\n- **Connection refused** (e.g., `ssh: connect to host iad1-shared-b8-40.dreamhost.com port 22: Connection refused`):\n  - SSH service is not running on remote host\n  - Firewall is blocking port 22\n  - Incorrect hostname or IP address\n  - Remote host not accessible\n  - Verify with: `ping iad1-shared-b8-40.dreamhost.com` and `nc -z iad1-shared-b8-40.dreamhost.com 22`\n\n- **Permission denied** (e.g., `Permission denied (publickey,password,keyboard-interactive)`):\n  - SSH key not properly installed or permissions incorrect\n  - Public key not in `~/.ssh/authorized_keys` on remote host\n  - SSH agent not running or key not loaded\n  - **Important**: You are using the correct DreamHost username (`agent_blue` with underscore, not `agent-blue` with dash)\n  - **Important**: Verify the exact hostname being used - some DreamHost configurations require the full domain while others work with subdomains\n\n#### Deploy with rsync\n```bash\nrsync -avz --delete -e \"ssh -i ~/.ssh/id_ed25519 -o BatchMode=yes\" site/ agent-blue@agent-blue.gitz.us:~/public_html/\n```\nNote: The `--delete` flag ensures the destination mirrors the source, removing files not present in the source.\n\n#### Deployment Verification\nAfter deployment, always verify the SSH connection worked by checking the rsync output for confirmation of transferred files. A \"Permission denied\" error indicates SSH authentication failed and the deployment did not occur.
